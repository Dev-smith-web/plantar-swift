...
class OptimizedARManager: NSObject, ARSessionDelegate, ObservableObject {
    ...
    @Published var tappedPart: PlantPart? = nil
    ...
    /// Called when student taps the AR view. Walks the entity hierarchy to find
    /// a named plant part and surfaces its details as a popup.
    func handleTap(at point: CGPoint, in arView: ARView, for plant: Plant) {
        guard let entity = arView.entity(at: point) else {
            tappedPart = nil
            return
        }

        // Walk up the entity tree — the hit entity may be a child mesh
        // whose named parent is the actual plant part (e.g. RayFlorets/RayFlorets)
        var current: Entity? = entity
        while let e = current {
            if let part = plant.plantParts.first(where: { $0.modelPartName == e.name }) {
                tappedPart = part
                highlightPart(named: e.name)
        ...
        // Tapped the model but landed on an unnamed mesh — dismiss popup
        tappedPart = nil
    }
